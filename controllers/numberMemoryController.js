const NumberMemory = require('../model/NumberMemoryRecoed');

const getAllRecordss = async (req, res) => {
    const recods = await NumberMemory.find();
...
const createRecord = async (req,res) => {
    ...
    const result = await NumberMemory.create({
        "username": username,
        "record": record
    });
    res.json(result)
...
const updateRecord = async (req,res) => {
    ...
    const record = await NumberMemory.findOne({ username: req.params.username }).exec();
    if (!record) {
