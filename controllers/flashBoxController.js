const FlashBox = require('../model/FlashBoxRecoed');

const getAllRecordss = async (req, res) => {
    const recods = await FlashBox.find();
...
const createRecord = async (req,res) => {
    ...
    const result = await FlashBox.create({
        "username": username,
        "record": record
    });
    res.json(result)
...
const updateRecord = async (req,res) => {
    ...
    const record = await FlashBox.findOne({ username: req.params.username }).exec();
    if (!record) {
