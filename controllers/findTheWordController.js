const FindTheWord = require('../model/FindTheWordRecord');

const getAllRecordss = async (req, res) => {
    const recods = await FindTheWord.find();
...
const createRecord = async (req,res) => {
    ...
    const result = await FindTheWord.create({
        "username": username,
        "record": record
    });
    res.json(result)
...
const updateRecord = async (req,res) => {
    ...
    const record = await FindTheWord.findOne({ username: req.params.username }).exec();
    if (!record) {
