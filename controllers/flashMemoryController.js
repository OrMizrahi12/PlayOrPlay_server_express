const FlashMemory = require('../model/FlashMemoryRecoed');

const getAllRecordss = async (req, res) => {
    const recods = await FlashMemory.find();
...
const createRecord = async (req,res) => {
    ...
    const result = await FlashMemory.create({
        "username": username,
        "record": record
    });
    res.json(result)
...
const updateRecord = async (req,res) => {
    ...
    const record = await FlashMemory.findOne({ username: req.params.username }).exec();
    if (!record) {
